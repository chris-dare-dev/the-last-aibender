...
[X4] automation slots ACTIVE in the templates since M4 (§7.1 amendment) ·
...
surface; §7.1 FROZEN-M4 — the [X4] routing amendment), plan §6/SI-3 + §9.2
SI-3 row, §9.3 BE↔SI #3/#4.
...
| Piece | Effect |
|---|---|
| `hooks` | 29 contract events, each a `type:"http"` POST (5 s timeout; SessionStart 10 s) to `http://127.0.0.1:4319/hooks/v1/<LABEL>`. Covers harness-launched **and** external sessions — this is how the context graph and reconciler see sessions the harness didn't spawn. The [X4] slots (`SessionStart`/`SessionEnd`/`PreCompact`) ride the same envelope and are ACTIVE since M4 (§"[X4] automation" below). |
...
## [X4] automation — the M4 slot activation (hooks-contract §7.1)

The three automation slots have ridden the template envelope since M2; at M4
they are ACTIVE per the frozen routing amendment. What the template encodes:

| Slot | Collector posture (§7.1) | Template window |
|---|---|---|
| `SessionEnd` | post-ack fire-and-forget → BE-7 auto continuation brief | 5 s (unchanged) |
| `PreCompact` | post-ack fire-and-forget → full-fidelity snapshot + `compact` edge | 5 s (unchanged) |
| `SessionStart` (matcher `startup\|resume\|clear\|compact`) | the ONE response-bearing slot: collector races BE-7's handler against its deadline and answers `200 + hookSpecificOutput.additionalContext` (the workstream's latest brief) or 204 | **10 s** — room for the deadline race; still short enough that a dead collector never stalls startup |

- Activation is **template-side only**: one plain http POST per slot, no
  second transport, no shell-outs [X2]. The brief injection rides the
  RESPONSE — the CLI applies the hook-output JSON natively.
- The per-account state file (`.aibender-hooks.json`) records the
  activation in its `x4` block, derived from the installed settings
  (asserted in lockstep by the bats suite). `live-check.sh --check
  x4-hook-slots` verifies the slots read-only on the live host.
- **Injection default is 204** (no injection): BE-7's handler output only
  ever reaches a session after the T3 item below lands.

**T3 verification item (hooks-contract §7.1, milestone gate, owner-run):**
the CLI-side interpretation of the SessionStart `200` body
(`additionalContext` injected as session context) MUST be verified against
the **pinned** CLI on the real host before brief injection turns on —
start a real session in one account (`claude` under that
`CLAUDE_CONFIG_DIR`), have the collector answer a fixture injection, and
confirm the brief text is present in the session context and NOWHERE else
(no file writes, no native-store mutation [X4]). Until then the collector
answers 204 and sessions behave exactly as at M3. The RESPONSE shape is
frozen either way.
