...
| [ICR-0017](icr-0017-events-sql-aggregation.md) | **ADDITIVE SQL-aggregation accessor over the frozen events store (finding OS-2).** New read-only `createEventsAggregatesStore(driver)` → `EventsAggregatesStore` (new file `eventsAggregates.ts`; the FROZEN-M3 `events.ts` write/dedupe path is untouched) + a new `EventsStore.eventsAggregates` bundle field. The eight window-scanning BE-6 leads now group/sum/count in SQLite (integer-exact leads) or take a narrow single-column scan (float-USD / percentile / burn leads) instead of materializing the whole window as `EventRow`s. **Byte-identical output** — no wire change, no protocol bump (`1.6.0`/FROZEN-M8 stand), no golden-corpus change: grouped queries preserve first-appearance order via `ROW_NUMBER() … MIN(rn)`; only integers are summed in SQL; float money keeps its exact per-row JS fold. No DDL migration (existing `events_ts_idx`/`events_account_ts_idx` serve the scans; a covering index was deferred to avoid collector write-amplification). Proven by `eventsAggregates.spec.ts` (400-row seeded corpus vs a JS-over-`list()` reference) + the existing hand-checked `projections.spec.ts` unchanged. **BE-ORCH: landed (additive, no freeze amendment); FE-ORCH: n/a (identical read-model snapshots).** | 2026-07-05 |
